...
async def get_item_type_by_id(info, id: strawberry.ID) -> ItemTypeScheme:
    db: AsyncSession = info.context["db"]
    return await ItemTypeService.get_by_id(db, id)


async def list_item_types(info) -> Sequence[ItemTypeScheme]:
    db: AsyncSession = info.context["db"]
...
async def create_item_type(info, input: ItemTypeInput) -> ItemTypeScheme:
    db: AsyncSession = info.context["db"]
    return await ItemTypeService.create(db, input)

async def update_item_type(info, id: strawberry.ID, input: ItemTypeInput) -> ItemTypeScheme:
    db: AsyncSession = info.context["db"]
    return await ItemTypeService.update(db, id, input)

async def delete_item_type(info, id: strawberry.ID) -> bool:
    db: AsyncSession = info.context["db"]
